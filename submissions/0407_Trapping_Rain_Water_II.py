class Solution:
    ...
    def trapRainWater(self, height_map):
        ...
        for i in range(num_of_rows):
            heapq.heappush(boundary, (height_map[i][0], i, 0))
            heapq.heappush(
                boundary,
                (height_map[i][num_of_cols - 1], i, num_of_cols - 1),
            )
            visited[i][0] = visited[i][num_of_cols - 1] = True

        for i in range(num_of_cols):
            heapq.heappush(boundary, (height_map[0][i], 0, i))
            heapq.heappush(
                boundary,
                (height_map[num_of_rows - 1][i], num_of_rows - 1, i),
            )
        ...
        while boundary:
            ...
            min_boundary_height, current_row, current_col = current_cell
